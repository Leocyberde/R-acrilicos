...
const SAMPLE_BUDGET = {
  ...
  notes: "Exemplo de observação do orçamento.",
};
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <input type="text" value={value || ""} onChange={e => onChange(e.target.value)}
        className="flex-1 px-2 py-1.5 rounded border border-slate-200 text-xs font-mono" />
    </div>
  );
}
...
function BudgetPreview({ config, settings }) {
  ...
  return (
    <div style={{
      fontFamily: c.body_font_family, fontSize: `${c.body_font_size}px`, color: c.body_color,
      background: "#fff", width: "210mm", height: "297mm",
      padding: `${c.page_padding}mm`, paddingBottom: "24mm",
...
<div style={{ fontSize: `${c.title_font_size}px`, fontWeight: c.title_bold !== false ? "bold" : "normal", color: c.title_color }}>{c.title_text || "Orçamento"} #{String(budget.id || "")}</div>
      ...
      <div style={{ marginBottom: "12px", lineHeight: "1.8" }}>
        {c.show_job !== false && <div><strong>JOB:</strong> {budget.job}</div>}
        {c.show_producer !== false && <div><strong>Produtor:</strong> {budget.producer}</div>}
        {c.show_client !== false && <div><strong>Cliente:</strong> {budget.client_name}</div>}
      </div>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <tbody>
          <tr style={{ borderTop: `${c.totals_border_thickness || 2}px solid ${c.totals_border_color || "#333"}`, borderBottom: `1px solid ${c.totals_border_color || "#aaa"}`, background: c.totals_bg }}>
            ...
            <td style={{ padding: "6px", fontWeight: "bold", textAlign: "right", width: "20%", color: c.totals_value_color }}>R${fmt(budget.total)}</td>
          </tr>
          <tr style={{ borderBottom: `${c.totals_border_thickness || 2}px solid ${c.totals_border_color || "#333"}`, background: c.totals_bg }}>
            ...
            <td style={{ padding: "6px", fontWeight: "bold", textAlign: "right", color: c.totals_value_color }}>R${fmt(budget.total_with_margin)}</td>
          </tr>
        </tbody>
      </table>
      {c.show_instructions !== false && c.instructions_text && (
        <div style={{ marginTop: "14px", padding: "8px 12px", border: `1px solid ${c.instructions_border_color}`, borderRadius: "4px", background: c.instructions_bg, fontSize: "11px", lineHeight: "1.7", whiteSpace: "pre-wrap", fontWeight: "bold", color: c.instructions_text_color }}>
          {c.instructions_text}
        </div>
      )}
      ...
      <div style={{ marginTop: "18px", padding: "10px 12px", border: `1px solid ${c.notes_border_color}`, borderRadius: "4px", background: c.notes_bg, fontSize: "11px", lineHeight: "1.7" }}>
        {budget.notes}
      </div>
      {c.show_footer !== false && (
...
export default function BudgetLayoutEditor() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (configId) {
        await localClient.entities.BudgetLayoutConfig.update(configId, { config_data: config });
      } else {
        const created = await localClient.entities.BudgetLayoutConfig.create({ config_data: config });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Editor de Layout — Orçamento</h1>
      ...
      <div className="flex gap-6 items-start">
        <div className="w-80 flex-shrink-0 bg-white rounded-xl border border-slate-200 p-5 space-y-0 overflow-y-auto max-h-[85vh] sticky top-4">
          ...
          <Section title="Tabela de Itens">
            ...
            <Field label="Mínimo de Linhas"><NumberInput value={config.blank_rows ?? 10} onChange={v => set("blank_rows", v)} min={0} max={50} /></Field>
          </Section>
            ...
            {config.show_instructions !== false && (
              <>
                <Field label="Texto das Instruções">
                  <textarea value={config.instructions_text || ""} onChange={e => set("instructions_text", e.target.value)} rows={5} className="w-full px-2 py-1.5 rounded border border-slate-200 text-sm resize-y" />
                </Field>
        ...
        <div className="flex-1 overflow-auto">
          <div style={{ transform: "scale(0.7)", transformOrigin: "top left", width: "calc(100% / 0.7)" }}>
            <BudgetPreview config={config} settings={settings} />
          </div>
        </div>
      </div>
    </div>
